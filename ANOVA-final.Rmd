...
```{r shapley-metric-specific-plots}
# Create separate plots for each metric with dataset subplots
create_metric_specific_shapley_plots <- function() {
  ...
  # Function to create data for one metric across all datasets
  create_metric_data <- function(target_metric) {
    results_data <- data.frame()
    
    for(ds in datasets) {
      result <- calculate_shapley_based_contributions(dataset, target_metric, ds)
      
      # Get all three Shapley values
      gan_shapley <- result$results$Shapley_Value[1]
      vae_shapley <- result$results$Shapley_Value[2]
      hybrid_shapley <- result$results$Shapley_Value[3]
      
      # Create long format data with proper model names
      dataset_data <- data.frame(
        Dataset = tools::toTitleCase(ds),
        Model = c("TimeGAN", "TimeVAE", "VRNNGAN"),  # Changed to proper names
        Shapley_Value = c(gan_shapley, vae_shapley, hybrid_shapley),
        stringsAsFactors = FALSE
      )
      
      results_data <- rbind(results_data, dataset_data)
    }
    
    return(results_data)
  }
  
  # Function to create plot for one metric
  create_metric_plot <- function(metric_name, metric_col, title) {
    data <- create_metric_data(metric_col)
    
    p <- ggplot(data, aes(x = Model, y = Shapley_Value, fill = Model)) +
      geom_bar(stat = "identity", alpha = 0.8, width = 0.7) +
      geom_text(aes(label = round(Shapley_Value, 3)), 
                vjust = -0.3, fontface = "bold", size = 3) +
      facet_wrap(~Dataset, scales = "free_y", ncol = 3) +
      labs(
        title = paste("Shapley Value Analysis:", title),
        subtitle = "Model contributions across datasets",
        x = "Generative Model",
        y = paste("Shapley Value (", title, ")"),
        fill = "Model"
      ) +
      theme_minimal() +
      theme(
      ...
      scale_fill_manual(
        values = c("TimeGAN" = "#6BAED6", "TimeVAE" = "#FD8D3C", "VRNNGAN" = "#74C476")  # Updated to match proper names
      ) +
  ...
  plots <- list(
    kl = create_metric_plot("KL Divergence", "kl", "KL Divergence"),
    wasserstein = create_metric_plot("Wasserstein Distance", "wasserstein", "Wasserstein Distance"),
    rmse = create_metric_plot("RMSE", "rmse", "RMSE"),
    mae = create_metric_plot("MAE", "mae", "MAE")
  )
  ...
  ggsave("shapley_kl_analysis.png", plots$kl, width = 12, height = 6, dpi = 300, bg = "white")
  ggsave("shapley_wasserstein_analysis.png", plots$wasserstein, width = 12, height = 6, dpi = 300, bg = "white")
  ggsave("shapley_rmse_analysis.png", plots$rmse, width = 12, height = 6, dpi = 300, bg = "white")
  ggsave("shapley_mae_analysis.png", plots$mae, width = 12, height = 6, dpi = 300, bg = "white")
  
  cat("All metric-specific Shapley plots saved!\n")
  cat("Files created:\n")
  cat("- shapley_kl_analysis.png\n")
  cat("- shapley_wasserstein_analysis.png\n") 
  cat("- shapley_rmse_analysis.png\n")
  cat("- shapley_mae_analysis.png\n")
  
  return(plots)
}

# Create the plots
metric_plots <- create_metric_specific_shapley_plots()
```